...
function submitFeedback(section, type) {
  ...
  const message =
    type === "helpful" ? "Merci pour votre retour positif !" : "Merci, nous prendrons en compte votre retour";
...
/**
 * Télécharge le PDF de la charte graphique
 */
function downloadPDF() {
  try {
...
/**
 * Fonction pour partager la charte
 */
function shareCharte() {
  if (navigator.share) {
    navigator
      .share({
        title: "Charte Graphique AK-Solutions",
        text: "Découvrez la charte graphique officielle d'AK-Solutions",
        url: window.location.href,
      })
      .catch(console.error);
  } else {
